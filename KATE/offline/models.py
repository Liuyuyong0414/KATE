...
def build_or_load_gen_model(args):
    ...
    if args.model_type in ['roberta', 'codebert']:
        ...
        encoder = AutoModel.from_pretrained(args.base_model, config=config)
        decoder_layer = nn.TransformerDecoderLayer(d_model=config.hidden_size, nhead=config.num_attention_heads)
        decoder = nn.TransformerDecoder(decoder_layer, num_layers=6)
        model = Seq2Seq(encoder=encoder, decoder=decoder, config=config,
                        beam_size=args.beam_size, max_length=args.max_test_tgt_length,
                        sos_id=tokenizer.cls_token_id, eos_id=tokenizer.sep_token_id)
    elif args.model_type in ['unixcoder']:
        config = AutoConfig.from_pretrained(args.base_model)
        tokenizer = AutoTokenizer.from_pretrained(args.base_model)
        model = AutoModel.from_pretrained(args.base_model)
    elif args.model_type in ['codegen']:
        config = CodeGenConfig.from_pretrained(args.base_model)
        tokenizer = CodeGenTokenizer.from_pretrained(args.base_model)
        if tokenizer.pad_token is None:
            tokenizer.pad_token = tokenizer.eos_token
        model = CodeGenForCausalLM.from_pretrained(args.base_model)
    else:
        config = AutoConfig.from_pretrained(args.base_model)
        tokenizer = AutoTokenizer.from_pretrained(args.base_model)
        model = AutoModelForSeq2SeqLM.from_pretrained(args.base_model)

    logger.info("Finish loading model [%s] from %s", get_model_size(model), args.base_model)
